...
const Dashboard = () => {
    ...
    const fetchProperties = async () => {
        ...
        try {
            ...
            const { data } = await api.get<{ properties: Property[] }>('/properties?limit=100');
    ...
    const filteredProperties = useMemo(() => {
        return properties.filter(property => {
            ...
            const matchesListing = listingTypeFilter === 'all' || property.listingType === listingTypeFilter;
            ...
            return matchesSearch && matchesType && matchesListing && matchesMinPrice && matchesMaxPrice && matchesCity;
        });
    }, [properties, searchQuery, typeFilter, listingTypeFilter, minPrice, maxPrice, cityFilter]);
    ...
    useEffect(() => {
        setPage(1);
    }, [searchQuery, typeFilter, listingTypeFilter, minPrice, maxPrice, cityFilter]);
    ...
    const hasActiveFilters = searchQuery !== '' || typeFilter !== 'all' || listingTypeFilter !== 'all' || minPrice !== '' || maxPrice !== '' || cityFilter !== 'all';
                ...
                <div className="space-y-8">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-brand-700 pb-8">
                        <div>
                            ...
                            <p className="text-brand-muted mt-1 text-sm">Discover your perfect home</p>
                        </div>
                    ...
                    {showFilters && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 p-6 bg-brand-900/50 border border-brand-700 rounded-2xl animate-in fade-in slide-in-from-top-4 duration-300">
                            <div className="space-y-2">
                                <label className="text-xs font-semibold uppercase tracking-wider text-brand-muted">City</label>
                                <select
                                    value={cityFilter}
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-semibold uppercase tracking-wider text-brand-muted">Property Type</label>
                                <select
                                    value={typeFilter}
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-semibold uppercase tracking-wider text-brand-muted">Listing Type</label>
                                <select
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-semibold uppercase tracking-wider text-brand-muted">Min Price (NPR)</label>
                                <input
                                    type="number"
                                    placeholder="0"
                                    value={minPrice}
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-semibold uppercase tracking-wider text-brand-muted">Max Price (NPR)</label>
                    ...
                    {!loading && filteredProperties.length > 0 && (
                        <div className="flex items-center justify-between">
                            ...
                            {hasActiveFilters && !showFilters && (
                                <button onClick={clearFilters} className="text-xs text-brand-muted hover:text-white underline underline-offset-4 decoration-brand-700 transition-all">
                                    Reset all filters
                                </button>
                            )}
                        </div>
                    ...
                    {loading ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
                            {[...Array(6)].map((_, i) => (
                                <div key={i} className="h-[420px] bg-brand-900/40 rounded-2xl border border-brand-800" />
                            ))}
                        </div>
                            ...
                            {totalPages > 1 && (
                                <div className="flex justify-center items-center gap-8 pt-10">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-brand-muted text-sm">{totalPages}</span>
                                    </div>
                                    <button
                        ...
                        <div className="text-center py-32 bg-brand-900/20 border border-dashed border-brand-700 rounded-3xl">
                            ...
                            <h3 className="text-xl font-semibold text-white">No properties match your criteria</h3>
                            <p className="text-brand-muted mt-2 text-sm max-w-xs mx-auto">Try adjusting your search terms or filters to find what you're looking for.</p>
                            <button
                                onClick={clearFilters}
