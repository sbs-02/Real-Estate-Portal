...
import type { Property, Favourite } from "../types/property";
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
              {[...Array(3)].map((_, i) => (
                <div
                  key={i}
                  className="h-[400px] bg-brand-800 rounded-2xl border border-brand-700"
                />
              ))}
            </div>
          ) : favourites.length > 0 ? (
            ...
            <div className="text-center py-32 border border-dashed border-brand-700 rounded-2xl">
              ...
              <p className="text-brand-muted mt-1 text-sm">
                Go to the dashboard and heart a property to save it.
              </p>
            </div>
          )}
        </div>
